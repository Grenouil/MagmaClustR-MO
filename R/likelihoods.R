...
logL_GP_mod <- function(hp,
  ...
  if(length(list_output_ID) > 1 && !is.character(kern)){
    ...
    if(length(list_output_ID) > 1 && is.character(kern)){
      # MO inversion of POST MEAN (for the mean process mu_0)
      ## Extract the input variables (reference Input + Covariates)
      all_inputs <- db %>%
      ...
      inv <- kern_to_inv(all_inputs, kern, hp, pen_diag)
    } else{
...
reconstruct_hp <- function(par_vector, hp_names, output_ids) {
  ...
  if (is_flattened_format) {
    ...
    n_hp_types <- length(hp_names)
    n_outputs <- length(output_ids)

    hp_matrix <- matrix(par_vector,
                        nrow = n_outputs,
                        ncol = n_hp_types,
                        byrow = TRUE)
